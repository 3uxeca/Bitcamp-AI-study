import tensorflow as tf
import numpy as np

tf.set_random_seed(777)

# Data
xy = np.array([[828.659973, 833.450012, 908100, 828.349976, 831.659973], 
                [823.02002, 828.070007, 1828100, 821.655029, 828.070007], 
                [819.929993, 824.400024, 1438100, 818.97998, 824.159973], 
                [816, 820.958984, 1008100, 815.48999, 819.23999], 
                [819.359985, 823, 1188100, 818.469971, 818.97998], 
                [819, 823, 1198100, 816, 820.450012], 
                [811.700012, 815.25, 1098100, 809.780029, 813.669983], 
                [809.51001, 816.659973, 1398100, 804.539978, 809.559998]]) 

# MinMaxScaler

def MinMaxScaler(data):
    numerator = data - np.min(data, 0)
    denominator = np.max(data, 0) - np.min(data, 0)
    # noise term prevents the zero division
    return numerator / (denominator + 1e-5)

xy = MinMaxScaler(xy)

x_data = xy[:, 0:-1] 
y_data = xy[:, [-1]]

print(x_data.shape, y_data.shape) # (8, 4), (8, 1)

X = tf.placeholder(tf.float32, [None, 4])
Y = tf.placeholder(tf.float32, [None, 1])

W = tf.Variable(tf.random_normal([4, 1]), name='weight')
b = tf.Variable(tf.random_normal([1]), name='bias')

# tf.nn.softmax computes softmax activations
# softmax = exp(logits) / reduce_sum(exp(logits), dim)
# hypothesis = tf.nn.softmax(tf.matmul(X, W) + b)

hypothesis = tf.matmul(X, W) + b

# cost/loss function
cost = tf.reduce_mean(tf.square(hypothesis - Y), axis=1) # compile

# Cross entropy cost/loss
# cost = tf.reduce_mean(-tf.reduce_sum(Y * tf.log(hypothesis), axis=1)) # categorical_crossentropy 수식
# train = tf.train.GradientDescentOptimizer(learning_rate=0.3).minimize(cost)
train = tf.train.AdamOptimizer(learning_rate=0.3).minimize(cost)
# train = tf.train.AdadeltaOptimizer(learning_rate=0.5).minimize(cost)
# train = tf.train.AdagradDAOptimizer(learning_rate=0.5).minimize(cost)
# train = tf.train.AdagradOptimizer(learning_rate=0.5).minimize(cost)
# train = tf.train.ProximalAdagradOptimizer(learning_rate=0.5).minimize(cost)
# train = tf.train.ProximalGradientDescentOptimizer(learning_rate=0.5).minimize(cost)

# Launch the graph in a session
sess = tf.Session()
# Initializes global variables in the graph.
sess.run(tf.global_variables_initializer())

for step in range(8001):
    cost_val, hy_val, _ = sess.run(
        [cost, hypothesis, train], feed_dict={X: x_data, Y: y_data})
    print(step, "Cost: ", cost_val, "\nPrediction:\n", hy_val)

sess.close()

pred = sess.run([hypothesis], feed_dict={X: x_data})
pred = np.array(pred) # 원본데이터와 형식 통일

y_data = y_data.reshape((-1,)) # flatten
pred = pred.reshape((-1,))

# RMSE 구하기
from sklearn.metrics import mean_squared_error
def RMSE(y_data, pred): # y_test, y_predict의 차이를 비교하기 위한 함수
    return np.sqrt(mean_squared_error(y_data, pred)) # np.sqrt 제곱근씌우기
print("RMSE : ", RMSE(y_data, pred))

# R2(결정계수) 구하기
from sklearn.metrics import r2_score
r2_y_predict = r2_score(y_data, pred)
print("R2 : ", r2_y_predict)

