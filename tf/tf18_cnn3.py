...
L3 = tf.nn.relu(L3)
# L3 = tf.nn.max_pool(L2, ksize=[1, 2, 2, 1],
#                     strides=[1, 2, 2, 1], padding='SAME')
...
L4_flat = tf.reshape(L4, [-1, 7 * 7 * 32]) # -1: 전체 행. flatten은 전부다 곱하는거!

##### 8/23 시작 ####
# Final FC 7x7x64 inputs -> 10 outputs
W5 = tf.get_variable("W5", shape=[7 * 7 * 32, 50],
                     initializer=tf.contrib.layers.xavier_initializer())
b1 = tf.Variable(tf.random_normal([50]))
L5 = tf.nn.relu(tf.matmul(L4_flat, W5) + b1)

W6 = tf.get_variable("W6", shape=[50, 40],
                     initializer=tf.contrib.layers.xavier_initializer())
b2 = tf.Variable(tf.random_normal([40]))
L6 = tf.nn.relu(tf.matmul(L5, W6) + b2)

W7 = tf.get_variable("W7", shape=[40, 10],
                     initializer=tf.contrib.layers.xavier_initializer())
b3 = tf.Variable(tf.random_normal([10]))
logits = tf.nn.softmax(tf.matmul(L6, W7) + b3)
