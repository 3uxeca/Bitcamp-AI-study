...
def build_network(optimizer='adam', activation='relu', encoding_dim=16):
    ...
    encoded = Dense(encoding_dim, activation=activation)(input_img) # 히든레이어
...
def create_hyperparameters():
    batches = [50, 100, 256]
    optimizers = ['adam', 'adadelta'] # 용도에 맞게 쓰자.
...
model = KerasClassifier(build_fn=build_network, verbose=3) 
...
search = RandomizedSearchCV(estimator=model,
                             ...
                             n_iter=10, n_jobs=1, cv=3, verbose=3)

search.fit(x_train, x_train, batch_size=100) # 데이터 집어넣기!

print(search.best_params_)

# 숫자들을 인코딩 / 디코딩
# test set에서 숫자들을 가져왔다는 것을 유의
encoded_imgs = build_network.encoder.predict(x_test)
decoded_imgs = build_network.decoder.predict(encoded_imgs)
...
history = build_network.autoencoder.fit(x_train, x_train, # x_train을 넣어서 x_train을 맞추게 한다.
