...
# # 사진 한 장 뽑기!
# pic = x_train[42] # 6만가지의 이미지 중 원하는 n번째 이미지를 보여줘라.
...
# input_img = Input(Conv2D(256, kernel_size=(3,3), input_shape=(32,32,3), activation='relu'))
# "encoded"는 입력의 인코딩된 표현
# encoded = Dense(encoding_dim, activation='relu')(input_img) # 히든레이어
encoded = Conv2D(3, (32,32), padding='same')(input_img)
# "decoded"는 입력의 손실있는 재구성 (lossy reconstruction)
decoded = Conv2D(3, (32,32), activation='softmax', padding='same')(encoded)
# decoded = Dense(3072, activation='softmax')(encoded)
...
# 인코딩된 입력을 위한 플레이스 홀더
# encoded_input = Input(shape=(32,32,3)) # 히든레이어를 decoding의 인풋레이어로 쓰겠다.
...
history = autoencoder.fit(x_train, x_train, # x_train을 넣어서 x_train을 맞추게 한다.
                          epochs=1, batch_size=256, verbose=2,
...
for i in range(n):
    ...
    plt.imshow(autoencoder_imgs[i].reshape(32, 32, 3))
    plt.gray()
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
plt.show()
