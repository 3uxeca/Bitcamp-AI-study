...
# pipe = make_pipeline(MinMaxScaler(), pipe)
...
# 랜덤 서치 --- (*2-1)
kfold_cv = KFold(n_splits=5, shuffle=True)
search = RandomizedSearchCV(estimator=pipe, param_distributions=parameters,
                            n_iter=10, n_jobs=1, cv=kfold_cv, verbose=1)
# search = RandomizedSearchCV(SVC(), parameters, cv=kfold_cv) # makepipeline용
