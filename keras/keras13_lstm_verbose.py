...
model = Sequential()
model.add(LSTM(20, activation = 'relu', input_shape=(3,1))) # 행 무시, dim = 3, 1개씩 잘라서 수행
...
model.fit(x, y, epochs=1000, verbose=0) # verbose=0:과정안보이기 =1:자세히 =2:간략히 =3:더간략히
...
yhat = model.predict(x_input, verbose=2) #predict의 경우 옵션이 0,1 둘 뿐이다.
print(yhat)
